...
const PopularTreksSection = () => {
  ...
  const maxIndex = Math.max(0, popularTreks.length - cardsPerView)

  // Auto-slide functionality - slide one card at a time, every 2 seconds
  useEffect(() => {
    if (isPaused) return
  ...
  useEffect(() => {
    const updateCardWidth = () => {
      const container = document.querySelector('.popular-treks-carousel-container')
      if (container) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-base md:text-lg text-gray-700 max-w-4xl mx-auto leading-relaxed">
            Discover our most loved & most popular Himalayan treks. As the best trekking company in Uttarakhand, we offer journeys from snow-covered mountains to flowering meadows. Perfect for nature enthusiasts, adventure seekers & first-time trekkers. With professional guidance and inclusive packages, explore the picturesque treks of Uttarakhand.
          </p>
        </div>

        {/* Carousel Container */}
        <div className="relative mb-8 popular-treks-carousel-container">
          {/* Navigation Arrows */}
          <button
            onClick={prevSlide}
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-black bg-opacity-70 hover:bg-opacity-90 rounded-full p-3 shadow-lg transition-all duration-300 hover:scale-110 active:scale-95"
          ...
          <button
            onClick={nextSlide}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-black bg-opacity-70 hover:bg-opacity-90 rounded-full p-3 shadow-lg transition-all duration-300 hover:scale-110 active:scale-95"
          ...
          >
            <div 
              ...
              style={{ 
                ...
                width: `${popularTreks.length * (cardWidth + 16)}px`
              }}
            >
